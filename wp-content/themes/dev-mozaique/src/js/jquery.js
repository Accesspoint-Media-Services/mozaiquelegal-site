jQuery(document).ready(function ($) {
    ...
    function startProgress($accordion, startFrom = 0) {
        clearInterval(progressInterval);
        
        const $progressBar = $accordion.find('.accordion-progress-bar');
    ...
    function switchToSection(newSectionIndex) {
        ...
        const $oldSection = $sections.eq(currentSectionIndex);
        ...
        const $newSection = $sections.eq(currentSectionIndex);
        const $firstAccordion = $newSection.find('.accordion-features[data-index="0"]');
    ...
    function openAccordion($section, index) {
        const $accordion = $section.find('.accordion-features[data-index="' + index + '"]');
    ...
    function startAutoPlay() {
        ...
        autoPlayTimer = setInterval(function() {
            ...
            const totalAccordions = $currentSection.find(".accordion-features").length;
            let $activeAccordion = $currentSection.find('.accordion-features-items:visible').closest('.accordion-features');

            if ($activeAccordion.length === 0) {
                $activeAccordion = $currentSection.find('.accordion-features[data-index="0"]');
            }            

            let currentAccordionIndex = $activeAccordion.length ? $activeAccordion.data('index') : -1;
            let nextAccordionIndex = currentAccordionIndex + 1;
    ...
    $(".accordion-features").on("mouseenter", function () {
        ...
        const $section = $accordion.closest('.accordion-section');
        const sectionIndex = $sections.index($section);
        const isActive = $accordion.find(".accordion-features-items").is(":visible");
    ...
    $(".accordion-features .accordion-features-heading").on("click", function () {
        const $accordion = $(this).closest('.accordion-features');
        const $section = $(this).closest('.accordion-section');
        const index = $accordion.data('index');
        const clickedSectionIndex = $sections.index($section);
        
        if (clickedSectionIndex !== currentSectionIndex) {
            const $otherSection = $sections.eq(currentSectionIndex);
    ...
    const startObserver = new IntersectionObserver(function(entries) {
        entries.forEach(function(entry) {
            if (entry.isIntersecting && !hasStarted) {
                hasStarted = true;
                const $firstAccordion = $sections.eq(0).find('.accordion-features[data-index="0"]');
                startProgress($firstAccordion);
                startAutoPlay();
            }
        });
    ...
    const sectionObserver = new IntersectionObserver(function(entries) {
        ...
        if (visibleSections.size > 0) {
            ...
            if (!visibleSections.has(currentSectionIndex)) {
                const newSection = visibleArray[0];
